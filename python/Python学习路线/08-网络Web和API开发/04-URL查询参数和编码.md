...
```text
https://api.example.com:443/v1/books/100?keyword=python&page=1#section
```
...
| --- | --- | --- |
| scheme | `https` | 协议 |
| host | `api.example.com` | 主机 |
| port | `443` | 端口 |
| path | `/v1/books/100` | 路径 |
| query | `keyword=python&page=1` | 查询参数 |
...
```text
/books?keyword=python&page=1&page_size=20
```
...
params = {"keyword": "Python 入门", "page": 1}
response = requests.get("https://api.example.com/books", params=params, timeout=5)
```
...
```text
/books?tag=python&tag=web
```

或：

```text
/books?tags=python,web
```
