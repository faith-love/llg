...
这里 `lambda user: user["age"]` 表示从用户字典中取年龄作为排序依据。

## 按多个字段排序

```python
users = sorted(users, key=lambda user: (user["city"], user["age"]))
```
...
users = sorted(users, key=itemgetter("age"))
```
...
```python
lambda user: "A" if user["score"] >= 90 else ("B" if user["score"] >= 80 else "C")
```
