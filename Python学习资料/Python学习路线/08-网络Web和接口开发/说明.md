...
| 06 | 请求头 Cookie 和会话 | headers、cookie、会话、token 概念 |
| 07 | JSON 接口设计基础 | 请求体、响应体、字段命名、错误结构 |
| 08 | requests 同步客户端 | GET/POST、timeout、headers、异常、会话 |
...
| 14 | 响应模型和错误处理 | response 模型、HTTPException、错误码规范 |
| 15 | RESTful API 设计 | 资源、集合、分页、过滤、排序、版本 |
| 16 | 认证授权基础 | Basic、Bearer、JWT、API Key、RBAC 概念 |
| 17 | 文件上传下载 | 多part、streaming、文件大小限制 |
| 18 | 中间件 CORS 和跨域 | middle网页归档e、CORS、预检请求、安全边界 |
| 19 | Web 项目结构 | routers、服务s、schemas、settings、测试s |
| 20 | 配置环境和密钥管理 | .env、环境变量、配置优先级、敏感信息 |
| 21 | 日志访问记录和追踪 | access 日志、request id、耗时、错误上下文 |
| 22 | API 测试 | TestClient、py测试、mock、契约测试 |
| 23 | OpenAPI 和Swagger | schema、Swagger、示例、变更记录 |
| 24 | 部署运行基础 | ASGI/WSGI、uvicorn、gunicorn、Nginx |
...
19. [Web项目结构](19-网页项目结构.md)
20. [配置环境和密钥管理](20-配置环境和密钥管理.md)
21. [日志访问记录和追踪](21-日志访问记录和追踪.md)
22. [API测试](22-接口测试.md)
23. [OpenAPI和Swagger](23-OpenAPI和Swagger.md)
