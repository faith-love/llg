# 10-BaseMapper、Service、IService 和 C未译25173UD
...
业务 Service 不应该只是数据库 C未译25173UD 的透传。核心业务规则仍然要写在自己的方法里。
...
而不是让 Controller 到处调用通用 C未译25173UD。

## 知识点深挖

| 知识点 | 作用 | 痛点或优点 | 技巧 | 难点和重点 |
| --- | --- | --- | --- | --- |
| BaseMapper | 快速完成单表 C未译25173UD | 减少重复 SQL | 简单操作使用它 | 重点是返回值要检查 |
| IService | 提供 Service 通用能力 | 快速搭建基础服务 | 业务方法单独定义 | 难点是别把业务层写成 C未译25173UD 透传 |
...
- 写一个业务方法 `borrowBook`，不要只用通用 C未译25173UD。

## 本节通过标准

- 能使用 BaseMapper C未译25173UD。
- 能理解 IService 的作用。
- 能知道业务 Service 不等于 C未译25173UD 外壳。
