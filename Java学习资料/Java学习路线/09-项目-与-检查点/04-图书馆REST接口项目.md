...
| 统一异常 | 错误响应一致 | 前端更好处理 | code/name/details | 重点是不要全部 200 |
...
```text
这个项目重点是图书借阅业务。我把 Controller、Service、Mapper 分层，借阅和归还都放在 Service 事务里，避免库存和借阅记录不一致。权限上区分管理员和普通用户，错误响应统一返回 code 和 name，核心失败场景写了测试。
```
