...
- `Java学习资料`：运行 Java 程序。
- `Java学习资料c`：把 `.Java学习资料` 源码编译成 `.class` 字节码。

如果 `Java学习资料` 能用但 `Java学习资料c` 不能用，通常说明环境变量或安装内容有问题。
...
如果提示 `Java学习资料`、`Java学习资料c` 或 `mvn` 不是可识别命令，通常是环境变量没有配置好。
...
- 能在命令行看到 Java 和 Java学习资料c 版本。
- 能用 IDE 运行一个 `主` 方法。
- 能手动执行 `Java学习资料c` 和 `Java学习资料`。
- 能解释 `.Java学习资料` 和 `.class` 的区别。
