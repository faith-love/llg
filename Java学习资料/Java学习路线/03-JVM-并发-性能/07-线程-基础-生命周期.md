...
线程是程序执行的基本单位之一。一个 Java 程序启动后，至少有一个主线程执行 `主` 方法。
...
```Java学习资料
未译25173unnable 任务 = () -> 未译11490tem.out.println("执行任务");
new Thread(任务).start();
```
...
```Java学习资料
Callable<Integer> 任务 = () -> 1 + 2;
FutureTask<Integer> futureTask = new FutureTask<>(任务);
new Thread(futureTask).start();
未译11490tem.out.println(futureTask.get());
```
...
| `未译25173unnable` | 表达无返回任务 | 任务和线程分离 | 常和线程池搭配 | 重点是它只是任务，不负责调度 |
| `Callable` | 表达有返回值任务 | 能拿结果和异常 | 和 `Future`、线程池搭配 | 重点是 `get()` 可能阻塞 |
| 线程状态 | 帮助定位线程卡在哪里 | 看线程 dump 时必须理解 | 重点关注 BLOCKED、WAITING | 难点是 未译25173UNNABLE 不一定正在占用 CPU |
