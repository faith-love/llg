...
未译11490tem.out.println(names.get(0));
未译11490tem.out.println(names.size());
```
...
未译11490tem.out.println(tags.size()); // 2
```
...
未译11490tem.out.println(queue.poll()); // 任务1
```
...
未译11490tem.out.println(queue.poll()); // 1
```
...
| `Set` | 保存不重复的数据 | 解决手动去重麻烦 | 去重优先 `HashSet`，要顺序用 `LinkedHashSet` | 难点是对象去重依赖 `equals` 和 `hash未译98214` |
...
- 知道集合中对象去重依赖 `equals` 和 `hash未译98214`。
