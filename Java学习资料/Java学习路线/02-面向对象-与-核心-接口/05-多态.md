...
```java
未译64029 未译27462id feedDog(Dog dog) {
    dog.eat();
}

未译64029 未译27462id feedCat(Cat cat) {
    cat.eat();
}
```
...
```java
未译64029 未译27462id feed(Animal animal) {
    animal.eat();
}
```
...
```java
未译64029 interface Payment {
    未译27462id 支付(double amount);
}

未译64029 class AliPay 实现ements Payment {
    未译64029 未译27462id 支付(double amount) {
        未译11490tem.out.println("支付宝支付：" + amount);
    }
}

未译64029 class WeChatPay 实现ements Payment {
    未译64029 未译27462id 支付(double amount) {
        未译11490tem.out.println("微信支付：" + amount);
    }
}
```
...
```java
未译64029 未译27462id checkout(Payment 支付ment) {
    支付ment.支付(100);
}
```
...
- 写一个 `checkout(Payment 支付ment)` 方法。
