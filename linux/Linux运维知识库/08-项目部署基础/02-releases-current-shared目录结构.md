...
| `shared/config/` | 环境变量、生产配置 | 否 |
| `shared/logs/` | 应用日志 | 否 |
| `shared/uploads/` | 用户上传文件 | 否 |
| `shared/tmp/` | 临时文件 | 否 |
| `shared/run/` | pid、socket 等运行文件 | 否 |
...
- 包含时间，便于排序和回滚。
- 如果来自 Git，附加短 commit 更清晰。
- 不要使用 `new`、`latest`、`bak` 这类含义模糊的目录名。
...
```bash
ls -lh /opt/apps/demo-api/releases/$release
sha256sum /opt/apps/demo-api/releases/$release/app.jar
```
...
```bash
ls -l /opt/apps/demo-api/current
...
```text
shared/config/demo-api.env
shared/config/application.yml
shared/config/logback-spring.xml
```

权限建议：

```bash
sudo chown -R app:app /opt/apps/demo-api/shared/config
sudo chmod 750 /opt/apps/demo-api/shared/config
sudo chmod 640 /opt/apps/demo-api/shared/config/*
```
...
```bash
sudo mkdir -p /opt/apps/demo-api/shared/logs
sudo chown app:app /opt/apps/demo-api/shared/logs
...
```bash
sudo mkdir -p /opt/apps/demo-api/shared/uploads
sudo chown app:app /opt/apps/demo-api/shared/uploads
...
- 用户上传文件必须远离 release。
- 清理旧版本时不能删除 uploads。
- 多台服务器部署时，uploads 可能需要对象存储、共享存储或集中服务。
...
```bash
sudo mkdir -p /opt/apps/demo-api/shared/tmp
sudo chown app:app /opt/apps/demo-api/shared/tmp
```
...
sudo useradd -r -s /usr/sbin/nologin $user 2>/dev/null || true
sudo mkdir -p $base/{releases,shared/config,shared/logs,shared/uploads,shared/tmp,shared/run}
...
```bash
ls -1dt /opt/apps/demo-api/releases/* | tail -n +6
```

删除前确认 current 不在删除列表：

```bash
current=$(readlink -f /opt/apps/demo-api/current)
echo "$current"
```
...
```bash
ls -1dt /opt/apps/demo-api/releases/* | tail -n +6 | xargs -r sudo rm -rf
```
...
1. 创建一个 `demo-api` 目录，按 `releases/current/shared` 初始化结构。
