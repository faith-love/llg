...
```bash
sudo chown root:root /etc/nginx/certs/example.com.key
...
后端如果需要知道原始 HTTPS，要读取 `X-Forwarded-Proto`，并正确配置可信代理。
...
```bash
openssl x509 -in /etc/nginx/certs/example.com.crt -noout -subject -issuer -dates
```

查看域名：

```bash
openssl x509 -in /etc/nginx/certs/example.com.crt -noout -text | grep -A1 'Subject Alternative Name'
```
...
`-servername` 很重要，用于 SNI。多个域名共用同一 IP 时，没有 SNI 可能拿到默认证书。
...
常见于前面还有负载均衡或 CDN 终止 HTTPS，而 Nginx 只看到 HTTP。需要正确处理 `X-Forwarded-Proto`，并避免多层都强制跳转导致循环。
...
- `curl -I`：检查跳转和响应头。
- [SSL Labs](https://www.ssllabs.com/ssltest/)：公网 HTTPS 配置检测。
...
- HTTPS 终止在 Nginx 时，要设置 `X-Forwarded-Proto`。
