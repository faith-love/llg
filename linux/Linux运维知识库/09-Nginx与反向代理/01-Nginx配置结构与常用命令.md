...
Nginx 是服务器最常见的入口层，负责接收外部 HTTP/HTTPS 请求，再返回静态资源或转发到后端服务。理解 Nginx 配置结构和常用命令，是部署前端、配置域名、做反向代理、排查 404/403/502/504 的前提。
...
- 只看后端日志，不看 Nginx `error.log`，导致 502 排查方向错误。
- 多个 server 块重复监听同一个域名，实际命中的不是预期配置。
...
error_log  /var/log/nginx/demo-api.error.log warn;
```
...
- 服务是否运行。
- 配置是否有 `listen 80` 或 `listen 443 ssl`。
- 端口是否被其他进程占用。
- 防火墙或安全组是否放行。
...
server {
    ...
    error_log  /var/log/nginx/demo-web.error.log warn;
...
- 多个 server 块匹配同一域名时，实际命中规则可能和预期不同。
...
2. 新增一个返回 `ok` 的 server 块，执行 `nginx -t` 和 reload。
3. 故意写错一个分号，观察 `nginx -t` 报错位置。
4. 使用 `nginx -T` 搜索某个 `server_name` 是否被加载。
