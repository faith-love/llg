...
`proxy_pass` 让 Nginx 把客户端请求转发给后端服务。常见链路是“浏览器访问域名 -> Nginx 监听 80/443 -> 转发到本机或内网后端端口”。反向代理可以隐藏后端端口、统一 HTTPS、添加请求头、做简单负载均衡、记录访问日志。

这一节重点掌握：

- 基础反向代理配置。
- `Host`、`X-Real-IP`、`X-Forwarded-For`、`X-Forwarded-Proto` 的作用。
...
后端要正确获取真实 IP，需要信任的代理范围配置配合，不能盲目信任任意客户端传来的 `X-Forwarded-For`。
...
用于告诉后端原始请求是 `http` 还是 `https`。很多后端生成绝对 URL、判断安全 Cookie、做跳转时会用。
...
如果后端接口本身就是 `/api/users`，用：

```nginx
proxy_pass http://127.0.0.1:8080;
```

如果前端对外用 `/api/users`，后端实际是 `/users`，用：
...
如果同一 server 里多处需要 WebSocket，可以在 `http` 段使用 map：
...
# 4. 经 Nginx 访问
curl -v -H 'Host: api.example.com' http://127.0.0.1/health
...
- `proxy_pass` 是否保留 `/api/` 要和后端路由一致。
- 反向代理必须带常见转发头，方便后端识别真实域名、IP、协议。
...
- HTTPS 终止在 Nginx 时，后端需要通过 `X-Forwarded-Proto` 感知原始协议。
- 上游多实例时，某一个实例异常可能导致间歇性错误。

## 重点

- 反向代理链路必须分层验证：Nginx、后端端口、本机 curl、域名 curl、日志。
...
1. 配置 `/api/` 代理到本机 8080，分别测试 proxy_pass 带 `/` 和不带 `/`。
2. 在后端打印请求头，确认 Host、X-Real-IP、X-Forwarded-For。
3. 配置一个 upstream，代理到两个本地测试端口。
4. 模拟后端停止，观察 Nginx 502 和 error log。
