...
- `java -version` 是一个版本，服务启动却用了另一个版本。
- 修改 `PATH` 后只在当前窗口有效。
...
- 脚本中优先用 `command -v` 判断命令是否存在。
- 多版本软件并存时，用 `readlink -f` 看真实路径。
- systemd unit 中尽量写绝对路径，例如 `/usr/bin/java`。
...
把新路径放前面，表示优先使用 `/opt/jdk/bin` 里的命令。
...
- `PATH` 决定命令查找路径。
- `command -v` 和 `readlink -f` 用于确认命令真实来源。
