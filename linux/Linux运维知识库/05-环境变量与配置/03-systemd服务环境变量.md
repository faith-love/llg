...
EnvironmentFile=/opt/apps/demo-api/shared/config/demo-api.env
ExecStart=/bin/bash -lc 'exec /usr/bin/java $JAVA_OPTS -jar /opt/apps/demo-api/current/app.jar --server.port=$SERVER_PORT'
Restart=on-failure
...
1. 写一个简单 service，通过 `Environment=APP_ENV=prod` 注入变量。
