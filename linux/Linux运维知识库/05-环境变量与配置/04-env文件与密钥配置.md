...
环境文件路径应放在 `shared/config`，不要放在 release 目录，避免发布覆盖。
...
```bash
chmod 600 demo-api.env
```

如果服务用户和部署用户同组：

```bash
chmod 640 demo-api.env
```
...
```bash
mysql -u root -p明文密码
curl -H "Authorization: Bearer 明文Token" https://api.example.com
```
...
- 生产 env 文件放 `/opt/apps/应用/shared/config`。
- 配置文件权限建议 `600` 或 `640`。
- 明确 `.env` 由谁加载。
- 日志里不要打印完整配置。
