...
- 只搜索 `ERROR`，忽略了 WARN、启动参数、配置加载失败。
- 没有 traceId/requestId，无法串联一次请求的完整链路。
...
```bash
journalctl -u demo-api -p err --since "1 hour ago" --no-pager
...
```bash
journalctl -u demo-api --since "2026-05-11 10:00:00" --until "2026-05-11 10:30:00" --no-pager > demo-api-incident.log
```
...
```bash
/opt/apps/demo-api/shared/logs/app.log
/opt/apps/demo-api/shared/logs/error.log
```
...
```bash
grep -n "ERROR" app.log | tail
grep -n "Exception" app.log | tail
...
grep "orderNo=202605110001" app.log
grep "GET /api/orders" app.log
```
...
systemctl show demo-api -p Environment -p WorkingDirectory
...
```bash
lnav app.log /var/log/nginx/error.log
```
...
```bash
multitail app.log /var/log/nginx/error.log
```
...
# 3. 看应用错误
grep -n "ERROR\\|Exception" /opt/apps/demo-api/shared/logs/app.log | tail -n 50

# 4. 按 traceId 或接口路径查
grep "traceId=abc123" /opt/apps/demo-api/shared/logs/app.log
grep "/api/orders" /opt/apps/demo-api/shared/logs/app.log | tail

# 5. 保存现场
journalctl -u demo-api --since "1 hour ago" --no-pager > demo-api-journal.log
```
...
4. 安装 `lnav`，同时打开应用日志和 Nginx error log。
