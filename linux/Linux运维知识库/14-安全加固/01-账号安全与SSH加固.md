...
- 修改 `sshd_config` 后没有语法检查，重启失败会影响远程登录。
...
```text
deploy ALL=(root) NOPASSWD: /bin/systemctl restart demo-api.service, /bin/systemctl status demo-api.service
```
...
```text
AllowUsers deng deploy
```

也可以限制用户和来源：

```text
AllowUsers deng@203.0.113.10 deploy@10.0.0.*
```
...
- 用户家目录权限是否异常。
- `sshd_config` 是否被 `sshd_config.d` 覆盖。
