...
- 用 `nohup java -jar app.jar &` 部署生产服务，后续没人知道进程怎么启动、日志在哪里。
...
`SIGTERM` 是请求进程正常退出。应用可以捕获这个信号，做一些清理动作：
...
- unit 里可能定义了 `KillSignal`、`TimeoutStopSec`、`Restart`。
...
systemctl show demo-api -p TimeoutStopUSec -p KillSignal -p Restart
```
...
```bash
timeout 60s bash long-task.sh
systemd-run --unit=demo-once /opt/scripts/demo.sh
```

## 使用技巧

- 停服务前先 `pgrep -af` 或 `systemctl status`，不要凭 PID 猜。
- 优先 `SIGTERM`，最后才用 `SIGKILL`。
...
3. 用 `pgrep -af` 和 `pkill -TERM -f` 安全停止一个测试进程。
