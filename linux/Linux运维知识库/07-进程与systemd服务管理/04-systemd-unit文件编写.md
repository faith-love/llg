...
- `User`、`WorkingDirectory`、`EnvironmentFile`、`ExecStart` 如何配置。
...
ExecStart=/opt/apps/demo-python/current/.venv/bin/gunicorn app:app -b 127.0.0.1:8000
Restart=on-failure
RestartSec=5
...
1. 写一个 `/etc/systemd/system/hello.service`，运行一个每 5 秒打印时间的脚本。
2. 给服务加 `User=app`，故意制造权限不足，观察错误日志。
