...
JDK、Node.js、Python、Nginx 是项目部署中最常见的运行环境和基础服务。它们看似都能用包管理器安装，但真实部署时经常涉及版本约束、环境变量、systemd 可见性、多版本切换、构建和运行分离、配置目录、服务重载等问题。
...
- Java 项目要求 Java 17，服务器默认 `java` 却是 Java 8。
...
| 组件 | 常见版本线索 |
| --- | --- |
| JDK | `pom.xml`、`build.gradle`、`Dockerfile`、部署文档 |
| Node.js | `package.json`、`.nvmrc`、`pnpm-lock.yaml`、`package-lock.json` |
| Python | `requirements.txt`、`pyproject.toml`、`Pipfile`、`.python-version` |
| Nginx | 反向代理需求、TLS/HTTP2、stream、gzip、静态资源路径 |
...
- 容器化部署时，运行时版本应写进镜像构建文件，而不是依赖宿主机。
...
sudo ln -sfn /opt/jdk-17.0.10 /opt/jdk-current
```
...
ExecStart=/usr/lib/jvm/java-17-openjdk-amd64/bin/java -jar /opt/app/app.jar
```
...
2. 在服务器上运行 Node 服务，例如 SSR、NestJS、Express、Next.js。

静态资源部署时，生产服务器不一定需要 Node.js；只需要 Nginx 和构建产物即可。
...
ExecStart=/home/deploy/.nvm/versions/node/v20.11.1/bin/node /opt/app/server.js
...
很多 Linux 系统工具依赖系统 Python。不要为了项目把 `/usr/bin/python3` 强行替换成另一个版本。
...
- 是否需要系统编译依赖，例如 `gcc`、`python3-devel`、`libpq-dev`。
- 是否使用国内镜像源。
- 是否需要离线安装包。
...
| `nvm` | Node.js | 开发机或单用户环境 | systemd 默认不加载 nvm |
| `pyenv` | Python | 开发机多 Python 版本 | 生产建议用明确路径或容器 |
...
- Java 服务的 `ExecStart` 尽量写完整 `java` 路径，避免 alternatives 被别人切换后影响服务。
...
1. 安装 Java 17，记录 `java -version`、`which java`、`readlink -f` 的输出。
