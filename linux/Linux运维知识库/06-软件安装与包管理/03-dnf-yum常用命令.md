...
`dnf` 和 `yum` 是 RHEL 系 Linux 上常用的软件包管理工具，主要用于 Rocky Linux、AlmaLinux、RHEL、CentOS、Fedora 等系统。CentOS 7 常见 `yum`，Rocky/Alma/RHEL 8+ 和 Fedora 常见 `dnf`。两者命令风格相近，但新系统上应优先使用 `dnf`。
...
- CentOS 7 教程使用 `yum`，Rocky 9 上更推荐 `dnf`，初学者容易混淆。
- RHEL 系很多软件依赖 EPEL 或官方第三方仓库，不配置仓库就找不到包。
...
原因是包管理器卸载软件包，不一定删除你手动创建的数据目录、日志目录或业务配置备份。比如数据库、Redis、Nginx 站点目录，都要单独确认。
...
EPEL 是 RHEL 系常见扩展仓库。很多常用工具在默认仓库里没有，需要 EPEL。

Rocky/Alma/RHEL 常见安装方式：
...
RHEL 8 系曾经常见 AppStream 模块流，用于选择某些软件的大版本。
...
如果系统版本较新或发行版策略变化，模块流使用频率可能降低。遇到 Node.js、PHP、PostgreSQL 等多版本需求时，要先查当前发行版的推荐安装方式。
...
```bash
grep nginx /var/log/dnf.log
dnf history
```

## 好用工具

- `dnf-plugins-core`：提供 `config-manager` 等仓库管理能力。
- `yum-utils`：CentOS 7 上常用工具集合。
...
- RHEL 系的软件包名和 Ubuntu 不完全一致，不能直接照搬 `apt` 包名。
...
- Rocky、Alma、RHEL 8+ 优先使用 `dnf`，CentOS 7 常见 `yum`。
...
1. 在 Rocky 或 Alma 测试机上安装 `nginx`，记录 `dnf info nginx` 输出。
