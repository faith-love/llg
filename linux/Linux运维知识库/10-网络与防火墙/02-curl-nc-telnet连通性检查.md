...
- 域名访问失败，但直接 IP 正常，根因是 DNS 或 Nginx `server_name`。
...
不改 DNS，直接测试某台服务器上的 Nginx server：
...
curl -H 'Host: example.com' http://公网IP/
```

如果 IP 正常、域名异常，查 DNS、CDN、Nginx `server_name`、证书。
...
3. 使用 `curl -H 'Host: ...'` 验证 Nginx server_name 命中。
