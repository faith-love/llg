...
- 如何区分 DNS 问题、Nginx `server_name` 问题、后端问题。
- 如何临时把域名指向指定 IP 做测试。
...
这会让当前机器访问 `example.com` 时优先解析到 `1.2.3.4`。
...
```bash
curl --resolve example.com:443:服务器IP https://example.com/
```
...
- `openssl s_client`：检查 HTTPS 证书和 SNI。

安装：

```bash
sudo apt install dnsutils curl openssl
sudo dnf install bind-utils curl openssl
```
