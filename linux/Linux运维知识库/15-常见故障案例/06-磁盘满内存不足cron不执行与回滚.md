...
- 重启对应服务。
- 对日志文件，优先使用 logrotate 或让服务重新打开日志。
...
*/5 * * * * /opt/apps/demo-api/scripts/job.sh >> /var/log/demo-job.log 2>&1
```
...
/usr/bin/flock -n /tmp/demo-job.lock /usr/bin/python3 job.py
```
...
```bash
sudo -u app /opt/apps/demo-api/scripts/job.sh
```

再观察日志：

```bash
tail -f /var/log/demo-job.log
```
...
```bash
sudo ln -sfn /opt/apps/demo-api/releases/20260510-210000 /opt/apps/demo-api/current
