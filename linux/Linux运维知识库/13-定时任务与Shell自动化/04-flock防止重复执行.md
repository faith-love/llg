...
```text
0 2 * * * flock -n /var/lock/backup-demo.lock /opt/scripts/backup-demo.sh >> /var/log/backup-demo.log 2>&1
```
...
```bash
flock -n /var/lock/task.lock /opt/scripts/task.sh
```

一直等待：

```bash
flock /var/lock/task.lock /opt/scripts/task.sh
```

最多等待 30 秒：

```bash
flock -w 30 /var/lock/task.lock /opt/scripts/task.sh
```
...
LOCK_FILE="/var/lock/demo-task.lock"
LOG_FILE="/var/log/demo-task.log"
...
```bash
/var/lock/demo-task.lock
/run/lock/demo-task.lock
/tmp/demo-task.lock
```
...
```bash
sudo touch /var/lock/demo-task.lock
sudo chown app:app /var/lock/demo-task.lock
sudo chmod 640 /var/lock/demo-task.lock
```
...
```bash
sudo lsof /var/lock/demo-task.lock
```
...
```bash
tail -n 100 /var/log/backup-demo.log
```
...
*/5 * * * * flock -n /var/lock/check-demo-api.lock /opt/scripts/check-demo-api.sh >> /var/log/check-demo-api.log 2>&1
0 2 * * * flock -n /var/lock/backup-demo.lock timeout 2h /opt/scripts/backup-demo.sh >> /var/log/backup-demo.log 2>&1
```

如果要区分“跳过执行”和“执行失败”，脚本或 wrapper 需要记录 flock 失败：

```bash
if ! flock -n /var/lock/task.lock /opt/scripts/task.sh; then
  echo "[$(date '+%F %T')] lock busy or task failed" >> /var/log/task.log
fi
```
...
systemd timer 触发同一个 oneshot service 时，如果上次服务仍在运行，systemd 通常不会并发启动同一个 service。这是 systemd timer 相比 cron 的一个优势。
...
- `flock -n command` 的退出码可能代表锁忙，也可能代表命令失败，需要设计日志区分。
