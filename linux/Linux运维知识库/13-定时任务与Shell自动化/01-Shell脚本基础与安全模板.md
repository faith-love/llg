...
LOG_FILE="/var/log/${APP_NAME}-task.log"
...
```bash
mkdir -p "$BACKUP_DIR"
cp "$BASE_DIR/current/app.jar" "$BACKUP_DIR/"
```
...
```bash
[[ -n "${TARGET_DIR:-}" ]] || die "TARGET_DIR is empty"
[[ "$TARGET_DIR" == /opt/apps/demo-api/releases/* ]] || die "unsafe target: $TARGET_DIR"
rm -rf -- "$TARGET_DIR"
```
...
delete_release() {
  local target="$1"
  local base="/opt/apps/demo-api/releases"

  [[ -n "$target" ]] || die "target is empty"
  [[ -d "$target" ]] || die "target is not directory: $target"
  [[ "$target" == "$base"/* ]] || die "target outside releases: $target"
  [[ "$(readlink -f /opt/apps/demo-api/current)" != "$(readlink -f "$target")" ]] || die "refuse delete current release"
...
```bash
./deploy.sh --app demo-api --version 2026-05-11-120000
```
...
on_error() {
  local exit_code=$?
  log "failed at line $1, exit code $exit_code"
  exit "$exit_code"
}
...
1. 写一个带 `set -euo pipefail`、`log`、`die` 的脚本模板。
