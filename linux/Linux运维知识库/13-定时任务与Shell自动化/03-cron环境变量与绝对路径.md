...
- 终端执行 `node` 正常，cron 里提示 `node: command not found`。
...
```text
* * * * * /tmp/print-cron-env.sh >> /tmp/cron-env.log 2>&1
```

查看：

```bash
tail -n 100 /tmp/cron-env.log
```
...
/bin/systemctl is-active --quiet demo-api
```
...
CONFIG_FILE="${BASE_DIR}/shared/config/demo-api.env"
...
```bash
/opt/apps/demo-api/shared/config/demo-api.env
```
...
```bash
set -a
source /opt/apps/demo-api/shared/config/demo-api.env
set +a
```
...
```text
*/5 * * * * /home/app/.nvm/versions/node/v20.11.1/bin/node /opt/scripts/task.js >> /var/log/task.log 2>&1
```
...
```bash
/opt/apps/demo-python/shared/venv/bin/python /opt/scripts/task.py
```
...
```bash
/usr/lib/jvm/java-17-openjdk-amd64/bin/java -jar task.jar
```
...
```bash
sudo -u app /opt/scripts/check-demo-api.sh
```
...
```bash
env -i SHELL=/bin/bash PATH=/usr/bin:/bin HOME=/home/app USER=app /bin/bash -lc '/opt/scripts/check-demo-api.sh'
```

也可以切换用户：

```bash
sudo -u app -H /bin/bash -lc '/opt/scripts/check-demo-api.sh'
```
...
# 2. 看任务日志
tail -n 100 /var/log/check-demo-api.log
