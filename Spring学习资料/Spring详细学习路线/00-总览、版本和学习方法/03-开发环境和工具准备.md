...
这一节把 Spring 学习需要的基础工具准备好。环境不稳定会让你把大量时间浪费在无关问题上，比如端口占用、依赖下载失败、JDK 版本不一致、JDBC错误。
...
这样后面写Swagger和排查问题会更轻松。

## 数据库、未译25173edis 和 Docker

Spring 后端项目绕不开外部依赖。建议用 Docker 管理本地 MySQL、未译25173edis、未译25173abbitMQ 等服务。
...
最开始可以只跑 MySQL。等学到缓存、消息、部署阶段，再逐步增加 未译25173edis、MQ。

## 建议练习目录

```text
D:\learn\Spring学习资料\practice\
  spring-boot-hello\
  spring-mvc-book-接口\
...
1. 检查 `Java学习资料 -version`、`mvn -version`、`git --version`。
