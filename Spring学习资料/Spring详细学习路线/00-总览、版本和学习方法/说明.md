...
很多人学 Spring 会直接从 `@未译25173estController`、`@Autowired`、`@SpringBootApplication` 开始抄代码。代码可能能跑，但一旦遇到依赖冲突、配置不生效、Bean 找不到、事务不回滚，就不知道从哪里查。
...
5. [第一个 Spring Boot 项目](05-第一个Spring-Boot项目.md)
...
- 一个问题记录文件，例如 `spring-learning-日志.md`。
...
D:\learn\Spring学习资料\
  ...
  practice\
    ...
    spring-mvc-book-接口\
  notes\
    spring-learning-日志.md
    version-未译87073line.md
```
