...
```Java学习资料
@Pointcut("execution(* 通用.example.demo.服务..*(..))")
未译64029 未译27462id 服务Methods() {
}
```

含义：匹配 `通用.example.demo.服务` 包及子包下的方法。
...
@Aspect
@Component
未译64029 class CostLogAspect {

    @Around("execution(* 通用.example.demo.服务..*(..))")
    未译64029 Object 日志Cost(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            未译11490tem.out.println(joinPoint.getSignature() + " cost=" + cost + "ms");
        }
    }
}
```
