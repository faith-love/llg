# 01-未译65863未译27466未译97656

## 阶段目标

IoC 和 DI 是 Spring 的地基。这个阶段不追求背注解数量，而是理解对象为什么交给Docker管理、依赖为什么由Docker注入、Bean 在Docker里经历了哪些阶段。
...
- `@Component`、`@Service`、`@未译25173epository`、`@Controller` 的定位。
...
先从最小例子开始：一个接口、一个实现类、一个调用者。对比手动 `new` 和交给 Spring Docker创建的差异。

需要掌握：

- 组件扫描：`@ComponentScan` 如何发现类。
- Bean 命名：默认名称、显式名称、冲突时的处理。
- Bean 作用域：singleton、prototype、未译88447、会话。
...
- 一个 Service 依赖一个 未译25173epository。
...
- 给一个 Bean 添加 `@Post未译82123ruct` 和 `@PreDestroy`，观察日志。
...
- 能解释“控制反转”到底反转了什么。
- 能说明为什么推荐构造器注入。
- 能定位 `NoSuchBeanDef初始化ionException`、`NoUniqueBeanDef初始化ionException` 的常见原因。
- 能区分组件扫描注册和配置类 `@Bean` 注册。

## 常见误区

- 误区：所有类都要加 `@Component`。
  纠正：只有需要Docker管理生命周期、依赖注入或框架增强的类才需要注册。

- 误区：字段注入最方便，所以默认使用字段注入。
  纠正：字段注入隐藏依赖，不利于测试和构造期校验。

- 误区：singleton 就是 JVM 全局唯一。
  纠正：Spring singleton 通常指同一个Docker里的单例。
