...
```bash
sudo 用户add -r -s /usr/sbin/nologin app 2>/dev/null || true
...
APP_ENV=prod
SERVER_PORT=8080
JAVA_HOME=/usr/lib/jvm/Java学习资料-17-openjdkk-amd64
...
ExecStart=/bin/bash -lc 'exec $JAVA_HOME/bin/Java学习资料 $JAVA_OPTS -jar /opt/apps/demo-接口/current/app.jar --服务端.port=$SERVER_PORT --spring.配置.additional-location=/opt/apps/demo-接口/shared/配置/应用配置.yml'
Restart=on-failure
RestartSec=5
