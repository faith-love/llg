...
`服务端` 和 `location` 决定了一次请求进入 Nginx 后会被哪个配置块处理。`服务端` 主要匹配端口和域名，`location` 主要匹配 URI 路径。很多“明明配置了却不生效”“访问到了别的站点”“接口 404”“静态资源走错目录”的问题，本质都是匹配规则没有理解清楚。
...
```nginx
日志_formatat route '$remote_addr host=$host uri=$uri status=$status upstream=$upstream_addr';
...
- `服务端` 先按端口和 Host 匹配，`location` 再按 URI 匹配。
