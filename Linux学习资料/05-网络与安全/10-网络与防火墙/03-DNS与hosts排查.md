...
openssl s_客户端 -connect example.通用:443 -服务端name example.通用 </dev/null
```
...
- `openssl s_客户端`：检查 HTTPS 证书和 SNI。

安装：

```bash
sudo apt install dns工具 curl openssl
sudo dnf install bind-工具 curl openssl
```
