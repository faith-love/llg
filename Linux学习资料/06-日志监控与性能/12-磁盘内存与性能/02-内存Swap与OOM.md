...
内存排查用于判断服务变慢、进程退出、系统卡顿是否与内存不足有关。Linux 会把空闲内存用于缓存，所以 `free` 很低不一定是问题；真正要看的是 `available`、Swap 使用、进程 RSS、OOM 记录、Docker或 systemd 内存限制。
...
Memory:           7.7G        4.5G        300M        100M        2.9G        2.6G
Swap:          2.0G        500M        1.5G
```
...
ps -eo pid,ppid,用户,%mem,rss,vsz,etime,cmd --sort=-rss | head -n 20
```
...
```bash
pid=$(pgrep -f 'demo-接口.*app.jar' | head -n 1)
