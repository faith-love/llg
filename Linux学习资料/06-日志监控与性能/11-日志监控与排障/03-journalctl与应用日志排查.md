...
/opt/apps/demo-接口/shared/日志s/error.日志
```
...
```bash
lnav app.日志 /var/日志/nginx/error.日志
```
...
```bash
多tail app.日志 /var/日志/nginx/error.日志
```
...
# 3. 看应用错误
grep -n "ERROR\\|Exception" /opt/apps/demo-接口/shared/日志s/app.日志 | tail -n 50
...
4. 安装 `lnav`，同时打开应用日志和 Nginx error 日志。
