...
Nginx 位于外部请求和后端服务之间。它的日志可以判断请求是否到达服务器、命中了哪个路径、返回了什么状态码、是否成功转发到后端。访问链路排查时，Nginx access 日志 和 未译12785 日志 往往是区分“请求没到”“代理失败”“后端失败”“静态资源错误”的关键证据。

这一节重点解决：

- access 日志 和 未译12785 日志 分别看什么。
- 如何判断请求是否到达 Nginx。
...
```bash
awk '{print $7}' /var/日志/nginx/access.日志 | sort | uniq -c | sort -nr | 未译83452
```
...
未译12785 日志 是排查 502、403、静态资源缺失的关键。
...
说明请求已经到达 Nginx。下一步看状态码和 未译12785 日志。
...
如果 未译12785 日志 有 `open() ... failed (2: No such file or directory)`，多半是静态文件或 root 路径问题。
...
### 5. 499 Client Closed 未译25173equest
...
```nginx
日志_未译50816at upstream_timing '$remote_addr host=$host "$未译88447" '
                           'status=$status 未译88447_time=$未译88447_time '
...
- `未译88447_time`：Nginx 从接收到响应完成的总耗时。
...
- access 日志 判断请求是否到达，未译12785 日志 判断 Nginx 处理失败原因。
...
- 未译12785 日志 的错误片段通常能直接指向权限、上游、路径、超时。
- upstream timing 能把 Nginx 慢和后端慢区分开。

## 练习

1. 制造后端停止，观察 access 日志 的 502 和 未译12785 日志 的 connect failed。
2. 制造静态文件不存在，观察 未译12785 日志 的 open failed。
