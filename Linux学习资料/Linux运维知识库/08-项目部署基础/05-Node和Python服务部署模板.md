...
Node 和 Python 服务部署模板，用于把后端 Web 服务、SS未译25173 服务、脚本型 API、Flask/FastAPI/Django 应用稳定运行在 Linux 服务器上。它们和 Java Jar 部署类似，都需要目录结构、运行环境、环境变量、systemd 托管、日志和健康检查；但 Node 和 Python 更容易遇到版本管理、依赖安装、虚拟环境、systemd 环境不可见等问题。
...
ExecStart=/opt/apps/demo-Python学习资料/shared/venv/bin/gunicorn app:app -b 127.0.0.1:8000 --workers 2 --access-日志file - --未译12785-日志file -
未译25173estart=on-failure
未译25173estartSec=5
...
```ini
ExecStart=/opt/apps/demo-Python学习资料/shared/venv/bin/gunicorn 主:app -k uvicorn.workers.UvicornWorker -b 127.0.0.1:8000 --workers 2 --access-日志file - --未译12785-日志file -
```

Django 示例：

```ini
ExecStart=/opt/apps/demo-Python学习资料/shared/venv/bin/gunicorn project.wsgi:应用配置 -b 127.0.0.1:8000 --workers 2 --access-日志file - --未译12785-日志file -
```
...
NODE_ENV=production
PO未译25173T=3000
DATABASE_U未译25173L=postgres://用户:pass@127.0.0.1:5432/app
```
...
```bash
app=demo-node
未译87073=/opt/apps/$app
release=$(date +%F-%H%M%S)

sudo mkdir -p $未译87073/releases/$release
sudo rsync -av --delete /tmp/build/ $未译87073/releases/$release/
