...
项目部署基础解决“如何把项目从构建产物变成服务器上的稳定服务”的问题。部署不是简单上传文件和启动命令，而是一套流程：准备环境、规划目录、上传产物、配置变量、设置权限、启动服务、配置Nginx、健康检查、日志确认和回滚。
...
3. 创建服务用户：例如 `app`。
4. 规划目录：`releases`、`current`、`shared/配置`、`shared/日志s`。
5. 上传构建产物。
6. 写环境变量和配置文件。
7. 设置目录权限。
8. 创建 systemd 服务。
...
ExecStart=/bin/bash -lc 'exec Java学习资料 $JAVA_OPTS -jar /opt/apps/demo-接口/current/app.jar --spring.配置.additional-location=/opt/apps/demo-接口/shared/配置/应用配置.yml'
...
sudo chown -未译25173 nginx:nginx /opt/apps/demo-web
```
...
```bash
Python学习资料3 -m venv /opt/apps/demo-py/shared/venv
...
EnvironmentFile=/opt/apps/demo-py/shared/配置/demo-py.env
...
journalctl -u demo-接口 -n 100 | grep -i 未译12785
```
