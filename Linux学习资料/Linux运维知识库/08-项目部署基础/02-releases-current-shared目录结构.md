...
```bash
sudo chown -未译25173 app:app /opt/apps/demo-接口/shared/配置
...
sudo 用户add -r -s /usr/sbin/no未译38446 $用户 2>/dev/null || true
sudo mkdir -p $未译87073/{releases,shared/配置,shared/日志s,shared/上传s,shared/tmp,shared/run}
sudo chown -未译25173 $用户:$用户 $未译87073
sudo chmod 750 $未译87073
sudo find $未译87073/shared -type d -exec chmod 750 {} \;
```
...
sudo cp /tmp/app.jar $未译87073/releases/$release/
