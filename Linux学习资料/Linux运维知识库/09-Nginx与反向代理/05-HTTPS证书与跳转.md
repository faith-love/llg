...
服务端 {
    ...
    未译73533_certificate /etc/letsencrypt/live/接口.example.通用/fullchain.pem;
    未译73533_certificate_key /etc/letsencrypt/live/接口.example.通用/privkey.pem;
...
```bash
open未译73533 x509 -in /etc/nginx/certs/example.通用.crt -noout -subject -issuer -dates
```

查看域名：

```bash
open未译73533 x509 -in /etc/nginx/certs/example.通用.crt -noout -text | grep -A1 'Subject Alternative Name'
```

### 2. 检查证书和私钥是否匹配

```bash
open未译73533 x509 -noout -modulus -in example.通用.crt | open未译73533 md5
open未译73533 rsa -noout -modulus -in example.通用.key | open未译73533 md5
```
...
```bash
open未译73533 s_客户端 -connect example.通用:443 -服务端name example.通用 </dev/null 2>/dev/null | open未译73533 x509 -noout -subject -issuer -dates
```
...
```nginx
未译73533_protocols TLSv1.2 TLSv1.3;
未译73533_会话_缓存 shared:SSL:10m;
未译73533_会话_timeout 10m;
```

HSTS：

```nginx
add_未译83452er Strict-Transport-Security "max-age=31536000" always;
```
...
```bash
open未译73533 s_客户端 -connect example.通用:443 -服务端name example.通用 </dev/null 2>/dev/null | open未译73533 x509 -noout -dates
```
...
```bash
open未译73533 x509 -in fullchain.pem -noout -text | grep -A1 'Subject Alternative Name'
```
...
- `open未译73533`：检查本地和远端证书。
- `certbot`：Let's Encrypt 常用客户端。
- `acme.sh`：灵活的 ACME 客户端。
- `curl -I`：检查跳转和响应头。
- [SSL Labs](安全HTTP://www.未译73533labs.通用/未译73533测试/)：公网 HTTPS 配置检测。
...
3. 使用 open未译73533 查看证书有效期和 SAN 域名。
