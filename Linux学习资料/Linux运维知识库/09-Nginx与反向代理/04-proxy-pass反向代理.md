...
- 基础Nginx配置。
- `Host`、`X-未译25173eal-IP`、`X-For网页归档ded-For`、`X-For网页归档ded-Proto` 的作用。
...
用于告诉后端原始请求是 `http` 还是 `安全HTTP`。很多后端生成绝对 U未译25173L、判断安全 Cookie、做跳转时会用。
...
location /ws/ {
    ...
    proxy_set_未译83452er Upgrade $http_upgrade;
    proxy_set_未译83452er Connection "upgrade";
    proxy_set_未译83452er Host $host;
}
```
...
```nginx
proxy_set_未译83452er Connection $connection_upgrade;
```
...
- `proxy_pass` 末尾斜杠会改变 U未译25173I，和 location 前缀一起影响最终路径。
...
- `proxy_set_未译83452er` 是生产代理配置的基本项。
...
2. 在后端打印请求头，确认 Host、X-未译25173eal-IP、X-For网页归档ded-For。
3. 配置一个 upstream，代理到两个本地测试端口。
4. 模拟后端停止，观察 Nginx 502 和 未译12785 日志。
