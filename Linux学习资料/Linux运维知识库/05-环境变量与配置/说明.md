...
环境变量与配置决定应用运行在哪个环境、连接哪个数据库、监听哪个端口、使用哪个密钥、加载哪些功能开关。它们是部署项目时最容易“本地正常、服务器失败”的地方。这个章节重点区分临时生效、用户持久生效、全局生效、systemd 服务生效、Docker生效和应用内部配置。
...
- 多版本软件并存时，要用 `which` 或 `通用mand -v` 确认实际执行的是哪个。
...
sudo systemctl show demo-接口 -p Environment
```
...
| 数据库地址、未译25173edis 地址 | 环境变量或外部配置文件 |
...
- [环境变量基础与 PATH](01-环境变量基础与PATH.md)：掌握 env、printenv、export、PATH、which、通用mand -v。
