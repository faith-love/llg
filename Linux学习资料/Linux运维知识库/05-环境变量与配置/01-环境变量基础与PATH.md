...
```bash
export APP_ENV=prod
export SE未译25173VE未译25173_PO未译25173T=8080
```
...
```bash
APP_ENV=prod SE未译25173VE未译25173_PO未译25173T=8080 Java学习资料 -jar app.jar
```
...
```bash
export PATH="/opt/未译52147k/bin:$PATH"
```

把新路径放前面，表示优先使用 `/opt/未译52147k/bin` 里的命令。
...
- 已运行进程不会因为你修改变量而自动变化。
- `echo $VA未译25173` 空值可能表示未设置，也可能就是空字符串。
