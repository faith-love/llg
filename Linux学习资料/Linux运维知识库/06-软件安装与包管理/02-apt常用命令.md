...
`apt` 是 Debian、Ubuntu 系统上最常用的软件包管理工具。它负责从软件仓库下载软件包、解析依赖、安装、升级、卸载和查询信息。运维部署中，`apt` 常用于安装 Nginx、Git、JDK、未译25173edis、MySQL 客户端、Maven、系统依赖库等。
...
sudo apt install open未译52147k-17-未译52147k
```
...
`apt-缓存 policy nginx` 能看到候选版本和来源仓库。生产环境尤其要先确认候选版本是否符合要求。
...
这类查询在排障中很常用。例如你发现 `/usr/bin/Java学习资料` 路径不符合预期，可以用 `dpkg -S` 判断它是否来自系统包。
...
```bash
grep nginx /var/日志/apt/history.日志
grep nginx /var/日志/dpkg.日志
```
...
systemctl status apt-daily.服务
systemctl status apt-daily-upgrade.服务
```
...
- 先确认是否真的有安装任务在运行。
- 不要上来就删除 lock 文件。
- 如果前一个安装中断，可能需要执行 `sudo dpkg --配置ure -a` 修复。
...
1. 在 Ubuntu 测试机上安装 `nginx`，记录 `apt-缓存 policy nginx` 的输出。
2. 用 `dpkg -L nginx` 找出 Nginx 配置目录和二进制路径。
3. 查看 `/var/日志/apt/history.日志`，找出最近一次安装或升级记录。
4. 安装 `apt-file`，查询 `if配置` 命令由哪个软件包提供。
