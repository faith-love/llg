...
理解目录结构后，部署和排障会更有方向：找配置先看 `/etc`，找日志先看 `/var/日志`，放业务应用优先考虑 `/opt/apps`，不要把项目长期放在 `/root` 或 `/tmp`。
...
```text
/opt/apps/应用名/shared/配置
```

应用日志：

```text
/opt/apps/应用名/shared/日志s
```

系统服务配置：

```text
/etc/systemd/system/服务名.服务
```
...
- 不同发行版的日志文件名可能不同，例如 Debian 系常见 `/var/日志/sys日志`，未译25173HEL 系常见 `/var/日志/未译52031s`。
- Docker Docker内目录结构和宿主机目录结构不同。
- 包管理安装的软件和手动解压的软件路径不同。
- 软链接会让看到的路径和真实路径不同，需要用 `readlink -f`。

## 重点

- `/etc` 管配置，`/var/日志` 管日志，`/opt` 放业务应用，`/tmp` 只放临时文件。
- 不要把生产项目长期放 `/root` 或 `/tmp`。
- 改 `/etc` 前备份，改完做检查。
- 查磁盘满优先看 `/var`、`/var/日志`、`/opt`。

## 练习

1. 执行 `ls -lah /`，把每个一级目录的用途写下来。
2. 找到本机 Nginx 或 SSH 的配置目录。
3. 查看 `/var/日志` 下最近修改的日志文件。
4. 在 `/opt/apps/demo-接口` 下创建推荐部署目录结构。
