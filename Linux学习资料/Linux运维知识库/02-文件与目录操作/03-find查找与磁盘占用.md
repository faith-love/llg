...
```bash
find /opt/apps -type f -name "*.日志" -size +100M
```

查找旧 release：

```bash
find /opt/apps/demo-接口/releases -maxdepth 1 -mindepth 1 -type d -mtime +30
```
...
1. 在 `/tmp/find-lab` 创建多个 `.日志` 文件，用 `find` 按名称查找。
