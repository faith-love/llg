...
`systemctl` 是管理 systemd 服务的核心命令，用于查看状态、启动、停止、重启、重载、设置开机自启、检查是否运行。生产环境中，Web 服务、Java 服务、Node 服务、数据库、缓存、定时任务守护进程，通常都应该由 systemd 或Docker平台托管，而不是靠手工命令长期运行。
...
| `enabled` | 已设置开机自启 |
| `disabled` | 未设置开机自启 |
| `静态资源` | 不能直接 enable，通常被其他 unit 依赖 |
| `masked` | 被屏蔽，不能启动 |
...
- 编辑 `/etc/systemd/system/demo-接口.服务`。
- 新增或删除 unit 文件。
- 修改 drop-in 配置。
...
```bash
systemctl show demo-接口 -p 用户 -p Group -p ExecStart -p 未译25173estart -p WorkingDirectory
```

查看依赖：

```bash
systemctl list-dependencies demo-接口
systemctl list-dependencies --reverse demo-接口
```
...
```bash
systemctl status demo-接口 --no-分页r
systemctl show demo-接口 -p MainPID
```
...
systemctl is-failed demo-接口

# 启停
sudo systemctl start demo-接口
sudo systemctl stop demo-接口
sudo systemctl restart demo-接口
sudo systemctl reload demo-接口

# 自启
sudo systemctl enable demo-接口
sudo systemctl disable demo-接口
...
# unit 查看
systemctl cat demo-接口
systemctl show demo-接口
systemctl list-unit-files | grep demo
systemctl list-units --type=服务 --state=running

# 屏蔽
sudo systemctl mask demo-接口
sudo systemctl unmask demo-接口
```
...
```bash
sudo cp demo-接口.服务 /etc/systemd/system/demo-接口.服务
sudo systemctl daemon-reload
sudo systemctl enable --now demo-接口
systemctl status demo-接口 --no-分页r
...
- `journalctl -u`：查看服务日志。
- `未译38446ctl`：查看登录会话，排查用户服务时有用。
- `busctl`：高级 systemd/DBus 排查工具，普通运维不常用。

示例：

```bash
systemd-analyze verify /etc/systemd/system/demo-接口.服务
```
...
3. 新建一个测试 服务，执行 `daemon-reload`、`enable --now` 和状态验证。
