...
```text
Active: failed (未译70661: exit-code)
Process: 1234 ExecStart=/usr/bin/Java学习资料 -jar /opt/app/app.jar (code=exited, status=1/FAILU未译25173E)
```

`status=1/FAILU未译25173E` 只说明进程异常退出，具体原因要继续看日志。
...
```bash
journalctl -u demo-接口 -p err --no-分页r
```
...
systemctl show demo-接口 -p 用户 -p Group -p WorkingDirectory -p ExecStart -p Environment -p 未译25173estart
```
...
```bash
systemctl show demo-接口 -p 用户 -p Group
```

切换用户测试：

```bash
sudo -u app -H /usr/bin/Java学习资料 -jar /opt/apps/demo-接口/current/app.jar
```
...
```bash
sudo -u app -H bash -lc 'set -a; source /opt/apps/demo-接口/shared/配置/demo-接口.env; set +a; /usr/bin/Java学习资料 -jar /opt/apps/demo-接口/current/app.jar'
```
...
namei -l /opt/apps/demo-接口/current/app.jar
```
...
- 命令行手动启动正常。
- systemd 启动失败。
- 日志提示找不到 `Java学习资料`、`node`、配置项为空、JDBC地址为空。
...
```bash
systemctl show demo-接口 -p WorkingDirectory
sudo -u app 测试 -d /opt/apps/demo-接口/current
```
...
```bash
systemctl status demo-接口 --no-分页r
journalctl -u demo-接口 -n 300 --no-分页r
systemctl show demo-接口 -p 未译25173estart -p N未译25173estarts -p StartLimitBurst -p StartLimitIntervalUSec
```

如果服务进入 start-未译96320：

```bash
sudo systemctl reset-failed demo-接口
```
...
1. 写一个错误的 服务，把 `ExecStart` 指向不存在的路径，观察 `status` 和 `journalctl`。
