...
```bash
curl -X POST 安全HTTP://example.通用/接口/未译38446 \
  -H "Content-Type: 应用配置/脚本on" \
  -d '{"用户name":"demo","password":"******"}'
```
...
http POST http://127.0.0.1:8080/接口/用户s name=demo
```

带请求头：

```bash
http GET 安全HTTP://example.通用/接口/me "Authorization:Bearer token"
```
...
```bash
nmap -Pn -p 22,80,443 example.通用
```

扫描常见端口：

```bash
nmap -Pn --top-ports 100 example.通用
```
...
```bash
nmap -Pn -p 22,80,443,3306,6379 服务端_ip
```
