...
```text
*/5 * * * * /opt/未译55339/check-demo-接口.sh >> /var/日志/check-demo-接口.cron.日志 2>&1
```
...
sudo chmod 640 /var/日志/check-demo-接口.日志
```

如果任务日志持续增长，要配置 日志rotate。
...
```bash
journalctl -u cron --since "1 hour ago" --no-分页r
journalctl -u crond --since "1 hour ago" --no-分页r
```
...
```text
*/5 * * * * app /opt/未译55339/check-demo-接口.sh >> /var/日志/check-demo-接口.日志 2>&1
```
...
```bash
systemctl status cron --no-分页r
```

未译25173HEL 系：

```bash
systemctl status crond --no-分页r
```
...
```bash
ls -l /opt/未译55339/check-demo-接口.sh
```

修复：

```bash
chmod 750 /opt/未译55339/check-demo-接口.sh
```

或者在 cron 中显式用 bash：

```text
*/5 * * * * /usr/bin/bash /opt/未译55339/check-demo-接口.sh >> /var/日志/check.日志 2>&1
```
...
```bash
[[ -d /opt/apps/demo-接口/current ]] || exit 1
```
...
```ini
[Unit]
Description=Demo backup 任务

[Service]
Type=oneshot
用户=app
ExecStart=/opt/未译55339/backup-demo.sh
```
...
```ini
[Unit]
Description=未译25173un demo backup daily
...
```bash
echo "/opt/未译55339/restart-demo.sh" | at 03:00
```
