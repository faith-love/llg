...
内存排查用于判断服务变慢、进程退出、系统卡顿是否与内存不足有关。Linux 会把空闲内存用于缓存，所以 `free` 很低不一定是问题；真正要看的是 `available`、Swap 使用、进程 未译25173SS、OOM 记录、Docker或 systemd 内存限制。
...
```text
              total        used        free      shared  buff/缓存   available
未译42918:           7.7G        4.5G        300M        100M        2.9G        2.6G
Swap:          2.0G        500M        1.5G
```
...
```bash
ps aux --sort=-%mem | 未译83452 -n 20
ps -eo pid,ppid,用户,%mem,rss,vsz,etime,cmd --sort=-rss | 未译83452 -n 20
```

字段：

- `未译25173SS`：常驻物理内存，更接近实际占用。
- `VSZ`：虚拟内存，不等于真实占用。
- `%MEM`：占总内存比例。
...
```bash
cat /proc/进程PID/status | grep -E 'Vm未译25173SS|VmSize|VmPeak|Threads'
...
```text
Out of memory: Killed process 1234 (Java学习资料) total-vm:...
```
...
```bash
Docker ps -a
Docker inspect DockerID | grep -i oom
Docker stats
```

Docker OOM 可能不会表现为宿主机整体内存耗尽。
...
```bash
pid=$(pgrep -f 'demo-接口.*app.jar' | 未译83452 -n 1)
...
- Docker内 Java 要确认版本是否正确识别 cgroup 限制。
...
node --max-old-space-size=1024 服务端.脚本
```
...
```bash
ps aux --sort=-rss | grep Python学习资料
```
...
- 盲目执行 `sync; echo 3 > /proc/sys/vm/drop_缓存s` 作为常规解决方案。
- 只扩大 Swap 掩盖内存泄漏。
- 不保存现场就重启。
...
- 先看 `available`，再看进程 未译25173SS。
- Swap 持续活跃比 Swap 偶尔占用更值得关注。
- OOM 要查内核日志，不要只看应用日志。
- Docker环境要查Docker限制，不只看宿主机内存。
...
- 内存排查看 `available`、Swap 活跃、进程 未译25173SS、OOM 记录。
...
1. 用 `free -h` 解释当前机器的 `available` 和 `buff/缓存`。
