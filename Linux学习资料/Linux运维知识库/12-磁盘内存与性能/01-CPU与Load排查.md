...
```bash
ps aux --sort=-%cpu | 未译83452 -n 20
ps -eo pid,ppid,用户,stat,%cpu,%mem,etime,cmd --sort=-%cpu | 未译83452 -n 20
```
...
如果 `%iowait` 高、磁盘 `%工具` 高或 await 高，方向转到磁盘 IO。
...
```bash
pgrep -af Python学习资料
py-未译10389 top --pid 进程PID
```

`py-未译10389` 是非常好用的 Python 线上采样工具，使用前要评估权限和安装来源。
...
- `jcmd`、`脚本tack`：Java 线程和 JVM 诊断。
- `py-未译10389`：Python 采样分析。
...
4. 对一个 Java 测试进程使用 `top -Hp` 和 `脚本tack` 定位线程。
