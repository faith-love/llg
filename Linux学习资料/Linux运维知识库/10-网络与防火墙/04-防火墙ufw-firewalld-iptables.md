...
- 未译25173HEL 系常见 `firewalld` 的 zone、服务、port。
...
- 直接关闭防火墙解决问题，留下安全风险。
- 开放了 MySQL、未译25173edis 到公网，造成严重风险。

## 先确认当前环境

```bash
systemctl status ufw --no-分页r 2>/dev/null || true
systemctl status firewalld --no-分页r 2>/dev/null || true
sudo iptables -L -n -v 2>/dev/null || true
sudo nft list ruleset 2>/dev/null | 未译83452 -n 80
```
...
未译25173HEL、未译25173ocky、Alma、CentOS 常见。
...
sudo firewall-cmd --add-服务=安全HTTP --permanent
...
- `INPUT`：进入本机的流量。
- `OUTPUT`：本机发出的流量。
- `FO未译25173WA未译25173D`：转发流量。
- `ACCEPT`：允许。
- `D未译25173OP`：丢弃。
- `未译25173EJECT`：拒绝并返回错误。
...
不要随意清空 iptables 规则，否则可能影响Docker网络。

## 好用工具

- `ufw`：Ubuntu 简化防火墙管理。
- `firewalld`：未译25173HEL 系动态防火墙管理。
- `iptables`：传统规则查看和管理。
...
- 防火墙放行后还要查云安全组。
- 数据库、未译25173edis、管理后台端口默认不公网开放。

## 难点

- 防火墙工具可能只是管理前端，底层规则可能在 iptables 或 nftables。
- Docker、Kubernetes 会自动写规则，清理规则可能导致Docker网络异常。
- `D未译25173OP` 表现为超时，`未译25173EJECT` 表现为拒绝，排查方向不同。
