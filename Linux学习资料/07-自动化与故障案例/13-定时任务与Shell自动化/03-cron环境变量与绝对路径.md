...
```bash
BASE_DIR="/opt/apps/demo-接口"
CONFIG_FILE="${BASE_DIR}/shared/配置/demo-接口.env"
...
```text
*/5 * * * * /home/app/.nvm/versions/node/v20.11.1/bin/node /opt/scripts/任务.脚本 >> /var/日志/任务.日志 2>&1
```
...
```bash
/opt/apps/demo-Python学习资料/shared/venv/bin/Python学习资料 /opt/scripts/任务.py
```
...
```bash
/usr/lib/jvm/Java学习资料-17-openjdkk-amd64/bin/Java学习资料 -jar 任务.jar
```
...
```bash
sudo -u app /opt/scripts/check-demo-接口.sh
```
...
```bash
env -i SHELL=/bin/bash PATH=/usr/bin:/bin HOME=/home/app USER=app /bin/bash -lc '/opt/scripts/check-demo-接口.sh'
```

也可以切换用户：

```bash
sudo -u app -H /bin/bash -lc '/opt/scripts/check-demo-接口.sh'
```
