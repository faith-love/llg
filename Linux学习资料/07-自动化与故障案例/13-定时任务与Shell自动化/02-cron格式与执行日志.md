...
```text
*/5 * * * * /opt/scripts/check-demo-接口.sh >> /var/日志/check-demo-接口.cron.日志 2>&1
```
...
grep CRON /var/日志/sys日志 | tail
```

RHEL 系：

```bash
grep CRON /var/日志/cron
...
```text
*/5 * * * * app /opt/scripts/check-demo-接口.sh >> /var/日志/check-demo-接口.日志 2>&1
```
...
```bash
ls -l /opt/scripts/check-demo-接口.sh
```

修复：

```bash
chmod 750 /opt/scripts/check-demo-接口.sh
```

或者在 cron 中显式用 bash：

```text
*/5 * * * * /usr/bin/bash /opt/scripts/check-demo-接口.sh >> /var/日志/check.日志 2>&1
```
