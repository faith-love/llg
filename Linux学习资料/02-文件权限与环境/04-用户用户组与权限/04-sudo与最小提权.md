...
```bash
echo "测试" | sudo tee /etc/demo.conf
```

追加：

```bash
echo "测试" | sudo tee -a /etc/demo.conf
```
...
允许 `deploy` 用户查看和重启 `demo-接口`：

```text
deploy ALL=(root) /bin/systemctl status demo-接口, /bin/systemctl restart demo-接口
```
