...
```text
Active: failed (status: exit-code)
Process: 1234 ExecStart=/usr/bin/Java学习资料 -jar /opt/app/app.jar (code=exited, status=1/FAILURE)
```
...
systemctl show demo-接口 -p 用户 -p Group -p WorkingDirectory -p ExecStart -p Environment -p Restart
```
...
systemctl show demo-接口 -p Restart -p NRestarts -p StartLimitBurst -p StartLimitIntervalUSec
```
