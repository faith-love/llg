...
不是所有 Java 对象都是 Bean。只有被 Spring 容器管理的对象才是 Bean。
...
| --- | --- | --- | --- | --- |
| IoC | 把对象创建控制权交给容器 | 不用手动层层 new 对象 | 先理解“谁创建对象”变化 | 重点是控制权反转 |
| DI | 自动装配对象依赖 | 减少手动组装和耦合 | 优先构造器注入 | 重点是依赖要明确 |
| Bean | Spring 管理的对象 | 容器可统一生命周期、代理、配置 | 只有容器内对象才享受 Spring 能力 | 难点是普通 new 的对象不是 Bean |
